...
import { ShoppingCart, Package, TrendingUp, DollarSign, Search, Plus, Minus, X, CreditCard, Barcode, Pill, AlertTriangle, Calendar, Building2, Download, Eye, Edit, Trash2, Filter, FileText, Clock, CheckCircle, User, Users, MessageSquare, Send } from 'lucide-react';
...
const PharmacyDashboard = () => {
    ...
    const handleClearPrescription = async (prescriptionId) => {
        await updatePrescription(prescriptionId, { status: 'Cleared' });
    };
    ...
    const handleProcessPayment = async () => {
        ...
        const handlePaymentUpdate = async () => {
            ...
            const bill = financialRecords.find(fin => fin.prescriptionId === prescription.id);
            if (bill) {
                ...
                await addBill({
                    ...
                    paymentMethod: paymentMethod,
                    department: 'Pharmacy',
                    description: `Prescription Payment - ${prescription.medications?.length || 0} items`
                });
            }
        };
        handlePaymentUpdate();
        ...
        if (prescription.medications && prescription.medications.length > 0) {
            for (const med of prescription.medications) {
                ...
                if (item) {
                    const newStock = Math.max(0, item.stock - (med.quantity || 0));
    ...
    const handleSaveMedicine = (e) => {
        ...
        const newMedicine = {
            ...
            price: parseFloat(medicineForm.unitPrice) || 0,
            stock: parseInt(medicineForm.quantity) || 0,
        ...
        if (editingMedicine) {
            updateInventoryItem(editingMedicine.id, newMedicine);
        } else {
            addInventoryItem(newMedicine);
        }
